...
public class Ch47ArrayAlgorithmsEx1_6
{
    ...
    /**
     * Method that computes and returns the element in the data array that is
     * closest to zero.
     * 
     * Array may contain negative numbers... so beaware...
     * 
     * @param data  array of integers to evaluate
    ...
    public int[] Ex5_Smooth( int[] signal )
    {
        ...
        {
            System.out.println("signal: " + signal[j]);
        }

        // write out the result
        for ( int j = 0; j < smooth.length; j++ )
        {
            System.out.println("smooth: " + smooth[j]);
        }
    ...
    public double[] Ex6_DataTweeker( String fileName ) throws IOException
    {
        ...
        int N = scan.nextInt();
        double[] phTable = new double[N];
        
        double total = 0;
        for (int i = 0; i < N; i++) {
            phTable[i] = scan.nextDouble();
            total += phTable[i];
        }
        average = total / N;

        double mostDistant = phTable[0];
        int dInd = 0;
        for (int i = 1; i < N; i++) {
            if (Math.abs(phTable[i] - average) > (Math.abs(mostDistant - average))) {
                mostDistant = phTable[i];
                dInd = i;
            }
        }
        ...
        for (int i = 0; i < N; i++) {
            if (!(i == dInd)) {
                newAverage += phTable[i];
            }
        }
        newAverage = newAverage / (N-1);
